...
When this repository releases a new version (see `pyproject.toml`), update each Town project from its root:
...
2. Bump the version in `pyproject.toml` — this is the single source of truth for releases:
...
3. Update `latest_updated_skills` in `manifest.json` with the folder names of the skills you modified or added (for example `App-Architecture`, `Code-Review`)
4. Push to GitHub — developers can then run `apex-skills update` in their projects

When you change the CLI tool itself, bump `pyproject.toml` and tell the team to run `uv tool upgrade apex-ai-skills`.
